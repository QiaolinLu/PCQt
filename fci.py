...
class FCI_test(QWidget):
    ...
    def initUI(self):
        ...
        self.button.clicked.connect(lambda: self.FCI_algorithm(
    ...
    def FCI_algorithm(self, input_data, alpha, indep_test):
        ...
        # G, edges = fci(input_data, indep_test, alpha, verbose=False)
        # pdy = GraphUtils.to_pydot(G, edges)
        # pdy.draw()
        G = fci(input_data, indep_test, alpha, verbose = True)
        pgv_g = GraphUtils.to_pgv(G[0])
